...
import com.repository.BlogImageReponsitory;
...
public class BlogDetailController {
	@Autowired
	private BlogImageReponsitory blogImageReponsitory;
	...
	public List<BlogDTO> getAllBlogWithImage() {
		...
		for (Blog blog : blogs) {
			List<Blog_img> blogImages = blogImageReponsitory.findByBlog(blog);
...
//		blogDTO.setUpdate_time(blog.getUpdate_time().format(DateTimeFormatter.ofPattern("HH:mm:ss")));
		...
		List<Blog_img> blogImgs = blogImageReponsitory.findByBlog(blog);
		List<String> imgUrls = new ArrayList<>();
...
//			blog.setBlog_id(blogDTO.getBlog_id());
			...
			if (imgUrls != null && !imgUrls.isEmpty()) {
				...
				blogImageReponsitory.saveAll(blogImgs);
			}
		}
	}
